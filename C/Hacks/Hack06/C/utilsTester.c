...
* library. Also included are three known personal test cases.
*/
...
/**
 * Custom Test 1: Verify pure red edge conversion
 */
static void testRgbToCmykPureRed(void **state) {
    double c, m, y, k;
    rgbToCMYK(255, 0, 0, &c, &m, &y, &k);
    assert_true(isClose(c, 0.0) && isClose(m, 1.0) && isClose(y, 1.0) && isClose(k, 0.0));
}

/**
 * Custom Test 2: Verify black CMYK edge case
 */
static void testRgbToCmykBlack(void **state) {
    double c, m, y, k;
    rgbToCMYK(0, 0, 0, &c, &m, &y, &k);
    assert_true(isClose(c, 0.0) && isClose(m, 0.0) && isClose(y, 0.0) && isClose(k, 1.0));
}
/**
 * Custom Test 3: Verify CMYK to RGB round-trip white
 */
static void testCmykToRgbWhite(void **state) {
    int r, g, b;
    cmykToRGB(0.0, 0.0, 0.0, 0.0, &r, &g, &b);
...
int main(int argc, char **argv) {
    ...
    const struct CMUnitTest tests[] = {
            ...
            cmocka_unit_test(testRgbToCmykPureRed),
            cmocka_unit_test(testRgbToCmykBlack),
            cmocka_unit_test(testCmykToRgbWhite),
